...
enum Inbound(`type`: InboundType) extends AbstractInbound {
  ...
  case TrojanInbound(
    ...
    tls: InboundTLS,
    /**
     * Fallback server configuration. Disabled if `fallback` and `fallback_for_alpn` are
     * empty.
     */
  ...
  case TuicInbound(
    ...
    tls: InboundTLS,
    /**
     * QUIC congestion control algorithm One of: `cubic`, `new_reno`, `bbr`. `cubic` is
     * used by default.
     */
    ...
    /**
     * Enable 0-RTT QUIC connection handshake on the client side. Disabling this is
     * highly recommended, as it is vulnerable to replay attacks.
     */
    zero_rtt_handshake: Option[Boolean] = None,
    /**
     * Interval for sending heartbeat packets for keeping the connection alive. `10s` is
     * used by default.
     */
  ...
  case TunInbound(
    ...
    /**
     * Linux iproute2 rule start index generated by `auto_route`. `9000` is used by
     * default.
     */
     ...
     * In Android:
     *
     * Only local IPv4 connections are forwarded. To share your VPN connection over
     * hotspot or repeater, use [[VPNHotspot]].
     *
     * In Linux:
     *
     ...
     *
     * It may prevent some applications (such as VirtualBox) from working properly in
     * certain situations.
     */
     ...
     *
     * Performance may degrade slightly, so it is not recommended to enable on when it
     * is not needed.
     */
     ...
     *
     * Defaults to the `mixed` stack if the gVisor build tag is enabled, otherwise
     * defaults to the `system` stack.
     */
    ...
    /**
     * > When `strict_route` enabled, return traffic to excluded interfaces will not be
     * automatically excluded, so add them as well (example: `br-lan` and `pppoe-wan`).
     *
    ...
    /**
     * > Android user and package rules are only supported on Android and require
     * auto_route.
     *
